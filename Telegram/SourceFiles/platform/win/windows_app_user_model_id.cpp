...
#ifdef OS_WIN_STORE
const WCHAR AppUserModelIdRelease[] = L"IMTT.IMTTDesktop.Store";
#else // OS_WIN_STORE
const WCHAR AppUserModelIdRelease[] = L"IMTT.IMTTDesktop";
#endif // OS_WIN_STORE
const WCHAR AppUserModelIdAlpha[] = L"IMTT.IMTTDesktop.Alpha";
...
void cleanupShortcut() {
	...
	QString path = systemShortcutPath() + u"IMTT.lnk"_q;
	std::wstring p = QDir::toNativeSeparators(path).toStdWString();
...
bool validateShortcut() {
	...
	if (cAlphaVersion()) {
		path += u"IMTTAlpha.lnk"_q;
		if (validateShortcutAt(path)) {
			return true;
		}
	} else {
		const auto installed = u"IMTT Desktop/IMTT.lnk"_q;
		const auto old = u"IMTT Win (Unofficial)/IMTT.lnk"_q;
